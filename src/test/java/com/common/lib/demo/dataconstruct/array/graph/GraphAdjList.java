package com.common.lib.demo.dataconstruct.array.graph;
...
//https://yngzmiao.blog.csdn.net/article/details/79587768
public class GraphAdjList<E> implements IGraph<E> {
   ...
   /* public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        Map<String, Integer> data = new HashMap<>(); // <word,index>
        for (int i = 0; i < wordList.size(); i++) {
            data.put(wordList.get(i), i);
        }
        int min = 0;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < beginWord.length(); i++) {
                String tempStr;
                if (i == 0) {
                    tempStr =endWord.charAt(0)+beginWord.substring(1,)
                    continue;

                }
                tempStr = current.substring(0, i) + endWord.charAt(i) + current.substring(i + 1, beginWord.length());
            }
